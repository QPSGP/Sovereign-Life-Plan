...
/** POST /api/universa/grantees/[id]/delete — delete grantee */
export async function POST(
  ...
  try {
    const g = await prisma.universaDocumentGrantee.findUnique({ where: { id }, select: { documentId: true } });
    if (!g) return NextResponse.redirect(new URL("/admin/documents", req.nextUrl.origin));
    await prisma.universaDocumentGrantee.delete({ where: { id } });
