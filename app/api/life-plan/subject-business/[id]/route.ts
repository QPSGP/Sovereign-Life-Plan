...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!name) return NextResponse.redirect(new URL("/admin/life-plan/subject/" + id + "?error=missing", origin));
  try {
    await prisma.subjectBusiness.update({
      where: { id },
      data: { name, verb, noun, object, objective },
    });
    return NextResponse.redirect(new URL("/admin/life-plan/subject/" + id, origin));
  } catch (e) {
    console.error(e);
    return NextResponse.redirect(new URL("/admin/life-plan/subject/" + id + "?error=update", origin));
  }
}
